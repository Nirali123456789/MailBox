...
class SpamRepository(private val spamDao: SpamDao) {
    ...
    var pinListData: LiveData<ArrayList<Spam>> = MutableLiveData()
    var arrayList: ArrayList<Spam> = arrayListOf()
    private val _pinList: MutableLiveData<Spam> = MutableLiveData()
    val pin_data: MutableLiveData<Spam> get() = _pinList
            ...
            processMessages(user, label) { m ->
                launch {
                    fun fetchAndProcess() {
                        ...
                        try {
                            Log.d("TAG", "newidisd .d.c v: " + "?>>>" + label.id)

                            var message1: Message
                            var article4: Spam = Spam()
                            message1 =
                           ...
                           // article.thread_id = message1.threadId
                            ...
                            CoroutineScope(Dispatchers.Main).launch(Dispatchers.IO) {
                                ...
                                if (!article.title.isNullOrBlank() || !article.content.isNullOrBlank() || !article.author.isNullOrBlank()) {
                                    ...
                                    Log.d("TAG", "SpamREpository: "+article)
                                    spamDao.insert(article)
//                                    if (!from) {
...
//        }
    ...
    private fun String.parseAddress(): String {
     ...
     fun getSpamList(): LiveData<List<Spam>> {
        return spamDao.getAllMail()
    }
