...
class FullMessageRepository(private val pindao: FullMessageDao) {
    ...
    fun adddata(pin: FullMessage) {
        scope.launch {
            pindao.insertPin(pin)
        }
    }

    fun deletedata(pin: FullMessage) {
        scope.launch {
            pindao.deletePin(pin)
        }
    }

     fun getFullMessageList(): LiveData<List<FullMessage>> {
        return pindao.getAllMessage()
    }
    fun getMessage(sender_id:String): LiveData<FullMessage>{
        return pindao.getMessage(sender_id)
    }
