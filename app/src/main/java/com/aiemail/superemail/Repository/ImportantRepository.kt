...
class ImportantRepository(private val importantDao: ImportantDao) {
    ...
    var pinListData: LiveData<ArrayList<Important>> = MutableLiveData()
    var arrayList: ArrayList<Important> = arrayListOf()
    private val _pinList: MutableLiveData<Important> = MutableLiveData()
    val pin_data: MutableLiveData<Important> get() = _pinList
                ...
                launch {
                    fun fetchAndProcess() {
                        ...
                        try {
                            ...
                            var article4: Important = Important()
                            message1 =
...
//
                            ...
                            var article: Important = Important()
...
//            credential1.token.refr
    ...
    fun adddata(pin: Important) {
        scope.launch {
            importantDao.insert(pin)
        }
    }

    fun deletedata() {
        CoroutineScope(Dispatchers.Main).launch(Dispatchers.IO) {
            importantDao.DeleteMails()
    ...
    fun getImportantList(): LiveData<List<Important>> {
        return importantDao.getAllMail()
    }
